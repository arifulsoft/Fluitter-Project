...
//				
	...
	public User signinStudent(@RequestBody User s) {
		UserDA d = new UserDA();
		User st=d.studentSigninById(s.username);
		return st;
	}
	...
	// Admin-costomar-register in flutter---------------------------------------

		@PostMapping("/singUp")
		public User adminreg(@RequestBody User s) {
			UserDA da = new UserDA();
			User st = da.adminreg(s);
...
//	Food order table secarch by c_id-------------------------------------------------------------------
	...
	//--------------------order Show in flutter----------------
	
	List<Foodtransaction> orderlist = new ArrayList<>();
	
	@GetMapping("/odershowall")
	public List<Foodtransaction> showallorder() {
		FoodtransactionDA da = new FoodtransactionDA();
		orderlist  = da.showallorder();
		return orderlist;
	}
	
	
   //serchbyid in flutter----------------------------
	List<Customar> stlist1=new ArrayList<>();
	@GetMapping(value="/search/{nid}")
	
	public List<Customar> serach(@PathVariable("nid") String nid){
		CustomarDA d = new CustomarDA();
		stlist1=d.searchstudent(nid);
		return stlist1;
	}
	

	   //serchbyRoomNumber in flutter----------------------------
		List<Customar> stlist12=new ArrayList<>();
		@GetMapping(value="/search1/{room}")
		
		public List<Customar> serach(@PathVariable("room") int room_number){
			CustomarDA d = new CustomarDA();
			stlist12=d.searchstudent1(room_number);
			return stlist12;
		}
